...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // ------------------------------------------------------------
    // Simple in-memory cache for select endpoints
    // ------------------------------------------------------------
    var _selectCache = Object.create(null);

    async function fetchSelectCached(url) {
      if (_selectCache[url]) return _selectCache[url]; // Promise
      _selectCache[url] = fetch(url, {
        ...
        .catch(function (err) {
          delete _selectCache[url]; // retry next time
          throw err;
        });

      return _selectCache[url];
    }
    ...
    async function loadAssigneesSelect(selectEl, selectedId) {
      ...
      try {
        var items = await fetchSelectCached('/api/tasks/assignees/select');
    ...
    async function loadCommMethodsSelect(selectEl, selectedId) {
      ...
      try {
        var items = await fetchSelectCached('/api/tasks/taskpm-communication-methods/select');
    ...
    // ✅ TaskType select (DisplayType szűréssel) + TomSelect wait + cache
    ...
    // ✅ TaskStatus select (DisplayType szűréssel) + cache
    async function loadTaskStatusesSelect(selectEl, selectedId, displayType) {
      ...
      try {
        var url = '/api/tasks/taskstatuses/select?displayType=' + encodeURIComponent(displayType);
        var items = await fetchSelectCached(url);
    ...
    // ------------------------------------------------------------
    // Warmup cache (non-blocking)
    // ------------------------------------------------------------
    Promise.allSettled([
      fetchSelectCached('/api/tasks/assignees/select'),
      fetchSelectCached('/api/tasks/taskpm-communication-methods/select'),
      fetchSelectCached('/api/tasks/tasktypes/select?displayType=' + encodeURIComponent(DISPLAY_TYPE)),
      fetchSelectCached('/api/tasks/taskstatuses/select?displayType=' + encodeURIComponent(DISPLAY_TYPE))
    ]);
    ...
    async function openEditModal(id) {
      ...
      try {
        ...
        // ✅ Selectek párhuzamosan (gyorsabb) + cache
        await Promise.all([
