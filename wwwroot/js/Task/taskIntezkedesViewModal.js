...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    console.log('[taskIntezkedesViewModal] DOM loaded');
    ...
    async function renderTask(d) {
      ...
      const relatedPartnerName = d.relatedPartnerName ?? d.RelatedPartnerName ?? '';
      ...
      bodyEl.innerHTML = `
        <div class="row g-4">
          ...
          ${relatedPartnerName && String(relatedPartnerName).trim()
                    ? `<div><strong>Kapcsolt partner:</strong> ${esc(relatedPartnerName)}</div>`
                    : ''}
    ...
    async function openTaskView(taskId) {
      ...
      try {
        ...
        console.error('[taskIntezkedesViewModal] load failed', e);
