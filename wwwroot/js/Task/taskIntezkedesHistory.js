// wwwroot/js/Task/taskIntezkedesHistory.js
(function () {
  'use strict';

  console.log('[taskIntezkedesHistory] file loaded');

  const modalEl = document.getElementById('taskHistoryModal');
  if (!modalEl) {
    console.warn('[taskIntezkedesHistory] modal not found (#taskHistoryModal)');
    return;
  }
  ...
  if (!elLoading || !elContent || !elTitle || !elEmpty || !elList) {
    console.warn('[taskIntezkedesHistory] missing inner elements', {
  ...
  function render(items) {
    console.log('[taskIntezkedesHistory] render items:', items);
  ...
  async function fetchAudit(taskId) {
    ...
    console.log('[taskIntezkedesHistory] fetch:', url);
  ...
  function openModal() {
    try {
      ...
      console.warn('[taskIntezkedesHistory] bootstrap modal open failed', e);
    }
  }

  async function load(taskId) {
    elTitle.textContent = taskId ? `Intézkedés #${taskId}` : '';
    clearList();
    showLoading();
    showEmpty(false);
    ...
    try {
      ...
      console.error('[taskIntezkedesHistory] load failed:', e);
    }
  }

  // ✅ Event from task*LoadMore.js
  window.addEventListener('tasks:history', function (e) {
    console.log('[taskIntezkedesHistory] tasks:history event:', e);

    const id = e?.detail?.id ?? e?.detail?.taskId;
    const taskId = parseInt(id, 10);

    console.log('[taskIntezkedesHistory] parsed taskId:', taskId);
